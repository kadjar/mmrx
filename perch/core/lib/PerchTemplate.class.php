...
class PerchTemplate
{
	...
	public function render($content_vars, $index_in_group=false)
	{
		...
		/*
		$i = 0;
        while (strpos($contents, 'perch:url')>0 && $i<10) {
    		...
    		if ($count > 0) {		    
    			foreach($matches as $match) {
    				$url_tag = new PerchXMLTag($match[0]);
    			    $contents = $this->parse_url_tag($url, $url_tag, $match[0], $contents, $content_vars);
    			}	
    		}
		...
		$s 			= '/<perch:(?!(form|input|label|error|success|setting|url))[^>]*>/';
